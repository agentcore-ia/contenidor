...
export function registerDashboardRoutes(app) {
  ...
  app.get('/api/posts', wrap(async (req, res) => {
    ...
    const { data, error } = await supabase
      .from('generated_posts')
      .select('id, hook, body, cta, caption_instagram, caption_x, caption_linkedin, image_url, image_urls, status, render_error, template_id, content_type, visual_direction, background_idea, model, created_at, calendar_id, category_id, videos:post_videos(id, kind, status, video_url)')
      .eq('brand_id', brand.id)
      .order('created_at', { ascending: false })
      .limit(limit);
